...
const songsJsonPath = path.join(publicDir, 'songs.json');
  ...
  return new Promise((resolve, reject) => {
    db.all(
      ...
      (err, rows) => {
        ...
        fs.writeFile(songsJsonPath, JSON.stringify(rows || [], null, 2), (writeErr) => {
          if (writeErr) {
            reject(writeErr);
            return;
          }
...
db.serialize(() => {
  ...
  `);
  exportToJSON().catch((err) => app.log.error(err));
});
        ...
        async function (err) {
          if (err) {
            ...
            try {
              ...
              app.log.error(exportErr);
              resolve(reply.code(500).send({ error: 'Error al exportar songs.json.' }));
              return;
            }
